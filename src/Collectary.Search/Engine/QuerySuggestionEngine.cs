...
public class QuerySuggestionEngine
{
    ...
    public QuerySuggestionEngine(QueryLexer lexer) => _lexer = lexer;

    public IReadOnlyList<QuerySuggestion> Suggest(string text, int caret, SearchUiSnapshot snapshot)
    {
    ...
    private IReadOnlyList<Candidate> Candidates(Context context, string? fieldLabel, SearchUiSnapshot snapshot)
    {
        switch (context)
        {
            case Context.Field:
    ...
    private IEnumerable<Candidate> FieldCandidates(SearchUiSnapshot snapshot)
    {
        foreach (var field in snapshot.Fields)
            yield return new Candidate(field.Label, _writer.WriteValue(field.Label), QuerySuggestionKind.Field);
    }

    private IEnumerable<Candidate> OperatorCandidates(string? fieldLabel, SearchUiSnapshot snapshot)
    {
        if (fieldLabel is null || snapshot.Find(fieldLabel) is not { } field) yield break;
        foreach (var op in field.Operators)
            yield return new Candidate(Display(op), Display(op), QuerySuggestionKind.Operator);
    }

    private IEnumerable<Candidate> ValueCandidates(string? fieldLabel, SearchUiSnapshot snapshot)
    {
        if (fieldLabel is null || snapshot.Find(fieldLabel) is not { } field) yield break;
        var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        foreach (var value in field.ValueSuggestions)
            if (seen.Add(value))
