...
{
    public Expression<Func<Item, bool>>? Build(BoundNode? root) =>
        root is null ? null : Visit(root, wantUpperBound: true).Expr;

    private sealed record BoundFilter(Expression<Func<Item, bool>>? Expr, bool? Constant);

    private BoundFilter Visit(BoundNode node, bool wantUpperBound) => node switch
    {
        BoundAndNode and => CombineAnd(Visit(and.Left, wantUpperBound), Visit(and.Right, wantUpperBound)),
        BoundOrNode or => CombineOr(Visit(or.Left, wantUpperBound), Visit(or.Right, wantUpperBound)),
        BoundNotNode not => Negate(Visit(not.Operand, !wantUpperBound)),
        BoundConditionNode condition => VisitCondition(condition, wantUpperBound),
        _ => throw new InvalidOperationException($"Unknown bound node {node.GetType().Name}."),
    };

    private BoundFilter VisitCondition(BoundConditionNode condition, bool wantUpperBound)
    {
    ...
    private BoundFilter Negate(BoundFilter inner)
    {
        ...
        return new BoundFilter(
            Expression.Lambda<Func<Item, bool>>(Expression.Not(inner.Expr.Body), parameter), null);
    }

    private Func<Expression<Func<Item, bool>>, Expression<Func<Item, bool>>, Expression<Func<Item, bool>>>
        ...
        (left, right) =>
        {
            ...
            return Expression.Lambda<Func<Item, bool>>(merge(left.Body, rebound), parameter);
        };
