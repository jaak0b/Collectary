...
/// into this assembly at build time from the <c>COLLECTARY_ONEDRIVE_CLIENT_ID</c> and
/// <c>COLLECTARY_ANDROID_SIGNATURE_HASH</c> build environment variables, because the phone has no
/// access to the developer's environment at runtime. The hash must match the <c>msauth://</c> redirect
/// registered for the Android platform in the Azure app registration; MSAL prints the expected value
/// in its error message on the first sign-in attempt if it is wrong.
/// </summary>
